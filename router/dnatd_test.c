...
#define errexit(errcode,format,arg...) \
    do { \
...
int main(int argc, char *argv[])
{
    ...
    /* initialization */
    /* port array */
    retval = newPortArray(&pArray, 60, 30000);
    if (retval < 0)
	errexit(-1, "create port array failed");
    /* iplist */
    retval = createIPList(IPList);
    if (retval < 0)
	errexit(-1, "create IPList failed");

    do {
	/* input */
	printf("Input command: Type pub_port pvt_IP pvt_port:\n\t");
	scanf("%d %hd %s %hd", type, pub_port, ipStr, pvt_port);
	str2digit(pvt_ip, ipStr);
    ...
    if (retval < 0)
	errexit(-2, "delete IPList failed");
